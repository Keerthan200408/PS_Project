...
void Node1Receive(Ptr<Socket> socket)
{
    ...
    {
        Simulator::Schedule(MilliSeconds(100), [packet]() {
            ...
            NS_LOG_INFO("Node 1 sent control to Node 2: " << controlInstruction);
...
int main(int argc, char *argv[])
{
    ...
    // Create UDP socket on Node 0 to send to Node1
    ...
    // Open file for reading Simulink data from pipeline
    // inputFile.open("/tmp/wind_data_debug.txt");


    //setup the function call to recive data on node1
    SetupNode1Receiver(nodes.Get(1));

    // outputFile.open("/tmp/control_data_debug.txt")

    SetupNode2Receiver(nodes.Get(2));


    // Start receiving and printing the data from the pipe every 100ms
    ...
    // -xstd::ofstream clearFile("/tmp/control_data_debug.txt", std::ofstream::out | std::ofstream::trunc);
